...
The following example shows how to create a Micrometer Tracing `Tracer` by using OTel's in-memory exporter that does not send spans anywhere but shows the integration point with the OTel `SpanExporter` interface:
